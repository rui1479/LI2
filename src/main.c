...
void game_over(int monstrosMortos, int tempo_jogado, int m) {
    ...
    sleep(5);
    // depois dos 5 segundos, o jogo recomeça
}
...
void init_mob(struct class_obj *mob, char **map, struct class_obj *player)
{
    ...
    // procura uma posição válida para colocar o mob
...
{// função responsável por mover todos os mobs
    for (int y = 0; y < MAP_HEIGHT; y++)
    {
        for (int x = 0; x < MAP_WIDTH; x++)
        {// percorre o mapa à procura de um mob, e quando encontra, chama a função que move o mob
    ...
    for (int y = 0; y < altura; y++)
    {
        for (int x = 0; x < largura; x++)
        {
            if (map[y][x] == 'e' || map[y][x] == 'b' || map[y][x] == 'c')
                mvwaddch(win, y, x, map[y][x]); // coloca no screen o caracter específico de cada mob
        }
    }
}
...
bool mobProx(int x, int y, char **mapa, int raio)
{
    ...
    for (int i = startX; i <= endX; i++)// percorre a matriz em volta do jogador para saber se existem mobs próximos
    {
    ...
    // Nenhum mob encontrado dentro do raio, ou seja, não se pode dar queue ao som do mob
    return false;
}
...
bool ataque_melee(int px, int py, int dir, char **map, WINDOW *win)
{
    ...
    switch (direction) // verifica para que direção o jogador se encontra virado e atualiza guarda a posição na direção em frente ao jogador
    {
    case 1: // Cima
        targetX = playerX;
        targetY = playerY - 1;
    ...
    char targetBlock = map[targetY][targetX]; // guarda a posição onde se vai dar o ataque
    ...
    {
        map[targetY][targetX] = '*';// coloca na posição do mob o '*' que simboliza o cadáver do morto
        mvwaddch(win, targetY, targetX, map[targetY][targetX]); // coloca na janela o '*'
        monstrosMortos++;// incrementa o número de mortos pelo utilizador
        return true;// retorna true, o que significa que o jogador matou um mob, o que é necessário para saber qual som emitir
    }
    
    return false;// retorna false se não matou mob nenhum
}
...
{
    char **tempMap = (char **)malloc(height * sizeof(char *));// cria um mapa temporário
    for (int i = 0; i < height; i++)
    {
    ...
    for (int i = 2; i < height - 2; i++)
    {
        for (int j = 2; j < width - 2; j++)
        {
            if (map[i][j] != 'b' && map[i][j] != 'c' && map[i][j] != 'e') // não pode alterar as poições dos mobs
            {
    ...
    // Liberta a memória do mapa temporário
    for (int i = 0; i < height; i++)
    {
        free(tempMap[i]);
    }
...
{
    if (x >= 0 && x < MAP_WIDTH && y >= 0 && y < MAP_HEIGHT) // verifica se as coordenadas estão dentro dos limites do mapa
    {
        ...
        if (map[(int)y][(int)x] == 'e' || map[(int)y][(int)x] == 'b' || map[(int)y][(int)x] == 'c') 
            return false;// se tiver um mob em frente, retorna só false
    }
    mvwaddch(win, y, x, map[(int)y][(int)x]); // pinta a posição e retorna true se não encontrar obstáculos
    return true;
}
...
void desenhaVisao(struct class_obj *player, char **map, WINDOW *win, int visaoAtivada)
{
    ...
    if (visaoAtivada) // se visão estiver ativada, desenha o campo de visão do jogador
    {
        double anguloIncremento = M_PI / 64.0;
        for (double angulo = 0; angulo < 2.0 * M_PI; angulo += anguloIncremento)// percorre todos os angulos de 0 até 2*pi
        {
            for (int i = 1; i < MAP_RADIUS; i++) // percorre para cada ângulo, o raio da visão
            {
                double x = player->x + i * cos(angulo); // atualiza a posição x do jogador
                double y = player->y + i * sin(angulo); // atualiza a posição x do jogador
                if (verificaParedeNaDirecao(x, y, map, win)) // verifica se tem obstáculos em frente
                {
                    if (map[(int)y][(int)x] != 'e' && map[(int)y][(int)x] != 'b' && map[(int)y][(int)x] != 'c')
                        mvwaddch(win, y, x, map[(int)y][(int)x]); // se não tiver mobs à frente, "pinta" a posição
                }
                else
                {
                    break;
    ...
    else // senão estiver atuvada, desenha o mapa todo
    {
        for (int y = 0; y < altura; y++)
        {
            for (int x = 0; x < largura; x++)
            ...
            }// só é responsável por desenhar as paredes do mapa
        }
    }
}
...
int main()
{
    ...
    keypad(stdscr, TRUE);// habilita a utilização das setas
    savetty();// salva as configurações atuais do terminal
    ...
    leaveok(stdscr, TRUE);//garante que temos o controlo total da posição na tela 
    curs_set(0);// oculta o cursor na tela
    SDL_Init(SDL_INIT_AUDIO); // inicializa a libraria SDL responsável pelo audio 
    SDL_AudioSpec wavSpec1; // armazena as especificações do ficheiro de som
    ...
    SDL_AudioDeviceID deviceId1 = SDL_OpenAudioDevice(NULL, 0, &wavSpec1, NULL, 0);// abre o ficheiro para utilização
    ...
    while (!EXIT)
    {
        ...
        case ESTADO_GAME:
            ...
            init_pair(1, COLOR_WHITE, -1);  // Padrão
            init_pair(2, COLOR_YELLOW, -1); // Jogador
            // alocação do mapa em memória
            ...
            int visaoAtivada = 1; // valor que determina se o jogador tem a visão ativada ou não
            int move_Count = 4; // variável que define se vai spawnar um mob ou não
            int m = 0; // número de moves feitos pelo jogador

            int c; // variável que vai ter o input do utilizador
            do
            {
                // Limpa a janela
                werase(win);
                ...
                desenhaMobs(map, win); // desenha todos os mobs nas suas novas posições
                ...
                if (move_Count == 5) // verifica se o move_count é 5 para spawnar o mob
                {
                    init_mob(&mob, map, &player); // chama a função que spawna o mob
                    move_Count = 0;
                }
                ...
                case KEY_UP:
                    if (player.y > 2 && map[player.y - 1][player.x] != '#')
                    {
                        ...
                        player.direction = 1; // atualiza a direção do jogador
                        ...
                        }// os mobs só fazem barulho se estiverem no campo de visão do jogador
                ...
                case KEY_RIGHT:
                    ...
                    break;
                case 'p':
                    updateMap(MAP_WIDTH, MAP_HEIGHT, map); // dá update ao mapa
                    break;
                case 't':
                    visaoAtivada = !visaoAtivada; // ativa ou desativa a visão
                    break;
                case 'a':
                    ...
                    }// se matou dá queue ao som de matar
                    else // se não matou dá queue ao som default
                    {
                        SDL_QueueAudio(deviceId2, wavBuffer2, wavLength2);// coloca na fila os dados ara reproduzir o som
                        SDL_PauseAudioDevice(deviceId2, 0); // começa a reprodução do som
                    }
                    if (mobProx(player.x, player.y, map, MAP_RADIUS) == true)
                    {
                        SDL_QueueAudio(deviceId3, wavBuffer3, wavLength3);
                ...
                case 's':
                    ...
                    break;
                case 'l':
                    limpa_mortos(map, win);break; // remove os corpos dos mobs mortos
                case 'q': // sai do jogo
                    endwin();
                    return 0;
                default:
